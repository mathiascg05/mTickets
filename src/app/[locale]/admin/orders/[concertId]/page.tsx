...
import ReconcileFlow from "@/components/admin/reconcile/ReconcileFlow";
...
function ConcertOrdersPageInner() {
  ...
  const hasPagoMovilMethod = (concert.paymentMethods || []).some(
    (pm: { type: string }) => pm.type === "pago_movil",
  );
  const hasZelleMethod = (concert.paymentMethods || []).some(
  ...
  return (
    <div>
      ...
      {/* Reconciliation: un solo flujo (columnas del archivo o asistente) */}
      {(hasPagoMovilMethod || hasZelleMethod) && (
        <div className="bg-surface border border-border rounded-xl p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold">{t("admin.reconcileFlow.sectionTitle")}</h2>
              <p className="text-sm text-muted">
                {t(
                  hasPagoMovilMethod && hasZelleMethod
                    ? "admin.reconcileDescBoth"
                    : hasZelleMethod
                      ? "admin.reconcileDescZelle"
                      : "admin.reconcileDesc",
                )}
              </p>
            </div>
            <ReconcileFlow
              concertId={concertId}
              hasPagoMovil={hasPagoMovilMethod}
              hasZelle={hasZelleMethod}
